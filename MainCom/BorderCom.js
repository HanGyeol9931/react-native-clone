import React from "react";
import styled from "styled-components/native";
import { Entypo } from "@expo/vector-icons";

const BorderContainer = styled.View`
  min-height: 10%;
  /* background: red; */
  flex-direction: row;
  padding: 10px 10px;
  border-bottom-width: 0.25px gray;
`;
const BorderView = styled.TouchableOpacity`
  width: 85%;
  flex-direction: column;
  /* background-color: blue; */
`;
const BorderTitleView = styled.View`
  flex-direction: row;
  justify-content: space-between;
`;
const BorderTitle = styled.Text`
  padding: 0 0 0 5px;
  font-weight: 800;
  font-size: 16px;
  color: black;
`;
const BorderContent = styled.Text`
  padding: 0 5px;
  color: black;
`;
const Profile = styled.Image`
  width: 50px;
  height: 50px;
  border-radius: 50%;
`;

const BorderContents = (props) => {
  return (
    <BorderContainer>
      <Profile
        source={{
          uri: "data:image/jpeg;base64,",
        }}
      />
      <BorderView
        activeOpacity="1"
        onPress={() => {
          props.BorderPage(props.content, props.keys);
        }}
      >
        <BorderTitleView>
          <BorderTitle>닉네임1</BorderTitle>
          <Entypo
            onPress={() => {
              props.deleteContent(props.keys);
            }}
            name="cross"
            size={24}
            color="grey"
          />
        </BorderTitleView>
        <BorderContent>{props.content}</BorderContent>
      </BorderView>
    </BorderContainer>
  );
};

export default BorderContents;
